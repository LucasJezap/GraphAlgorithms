...
class Graph:                                     # adjacency lists
    def __init__(self, E, V):
        ...
        for (x, y, c) in E:
            ...
            if not self.backEdge(x,y):
                self.G[y].append((x,0,0))

    def backEdge(self, x,y):
        for (u,f,c) in self.G[y]:
            if(u==x): return True;
        return False;
...
tests = os.listdir("graphs/flow/")
print(tests)
successes=0
fullTime=0

for i in tests:
    with open('graphs/flow/' + i) as f:
        res = int(f.readline().split()[3])
    time1 = time.time()
    res2 = solution('graphs/flow/' + i)
    time2 = time.time()
    if res == res2:
        successes += 1
