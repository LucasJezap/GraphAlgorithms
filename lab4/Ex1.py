...
def is_PEO(G,V,vs):             # TODO: BETTER COMPLEXITY ?
...
tests = os.listdir("graphs/chordal/")
print(tests)
successes=0
fullTime=0

for i in tests:
    with open('graphs/chordal/' + i) as f:
        res = int(f.readline().split()[-1])
    time1 = time.time()
    res2 = solution('graphs/chordal/' + i)
    time2 = time.time()
    if res == res2:
        successes += 1
